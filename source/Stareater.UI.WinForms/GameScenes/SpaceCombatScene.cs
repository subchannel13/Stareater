	...
	class SpaceCombatScene : AScene
	{
		...
		private IEnumerable<PolygonData> planetSpriteData(CombatPlanetInfo planet)
		{
			...
			if (planet.Population > 0)
				foreach (var layer in TextRenderUtil.Get.BufferText(formatter.Format(planet.Population), -1, MoreCombatantsZ, 1 / Layers))
					yield return new PolygonData(
						layer.Key,
		...
		private IEnumerable<PolygonData> projectileSpriteData(IGrouping<Vector2D, ProjectileInfo> hex)
		{
			...
			var formatter = new ThousandsFormatter();
			foreach (var layer in TextRenderUtil.Get.BufferText(formatter.Format(shownProjectile.Count), -1, ProjectileZ, 1 / Layers))
				yield return new PolygonData(
					layer.Key,
					new SpriteData(
        ...
        private SceneObject unitSprite(IGrouping<Vector2D, CombatantInfo> hex, IEnumerable<PlayerInfo> players)
		{
			...
			var formatter = new ThousandsFormatter();
			polygons.AddRange(
				TextRenderUtil.Get.BufferText(formatter.Format(unit.Count), -1, CombatantZ, 1 / Layers).
				Select(layer =>
					new PolygonData(
						layer.Key,
						new SpriteData(
