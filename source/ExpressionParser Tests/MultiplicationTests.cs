...
namespace ExpressionParser_Tests
{
	...
	public class MultiplicationTests
	{
		...
		{
			var test = new ParserTester("x/3", new Var("x", 2), 2.0 / 3, 1e-9);
			Assert.IsTrue(test.IsOK, test.Message);
		}
		...
		{
			var test = new ParserTester("x%-3", new Var("x", 10), 2, 1e-9);
			Assert.IsTrue(test.IsOK, test.Message);
		}
		...
		{
			var test = new ParserTester("x\\3", new Var("x", -10), -4, 1e-9);
			Assert.IsTrue(test.IsOK, test.Message);
		}
		...
		{
			var test = new ParserTester("x*3", new Var("x", 2), 6);
			Assert.IsTrue(test.IsOK, test.Message);
		}
	}
}
