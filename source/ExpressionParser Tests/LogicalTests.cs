...
namespace ExpressionParser_Tests
{
	...
	public class LogicalTests
	{
		...
		{
			var test = new ParserTester("5 & 7 & x", new Var("x", 1), 1);
			Assert.IsTrue(test.IsOK, test.Message);
		}
		...
		{
			var test = new ParserTester("a ∧ 1", new Var("a", -1), -1);
			Assert.IsTrue(test.IsOK, test.Message);
		}
		...
		{
			var test = new ParserTester("-2 | -1 | x", new Var("x", 1), 1);
			Assert.IsTrue(test.IsOK, test.Message);
		}
		...
		{
			var test = new ParserTester("1 @ 1 @ -1 @ x", new Var("x", -1), -1);
			Assert.IsTrue(test.IsOK, test.Message);
		}
